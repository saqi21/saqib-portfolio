...
export const articles: Article[] = [
  {
    id: "14",
    slug: "lessons-i-learned-building-cypress-framework-real-mistakes-and-fixes",
    title:
      "Lessons I Learned While Building a Cypress Framework (Real Mistakes & Fixes)",
    subtitle: "Honest Reflections from the Beambox Project",
    description:
      "The real mistakes I made while building a Cypress automation framework from scratch — flaky selectors, bad structure, overcomplicated logic — and how I fixed every single one.",
    image: `${basePath}/images/articles/article11.png`,
    ...
    content: {
      intro:
        '<p>Let me be honest with you.</p><p>When I first started building a Cypress automation framework for <strong>Beambox</strong>, I thought I had it all figured out. I had read the docs, watched some tutorials, and felt confident that things would go smoothly. I was wrong. <mark>Very wrong.</mark></p><p>I made mistakes. Some small, some embarrassingly big. Tests that passed one day would fail the next for no obvious reason. My framework grew messy. I spent hours fixing things that should not have broken in the first place.</p><p>But here is the thing — <strong>those mistakes taught me more than any course or tutorial ever could</strong>. Every failure forced me to rethink my approach, dig deeper, and build something better.</p><p>In a previous article, I shared the story of how I built the Cypress framework from scratch — the tools I chose, the results we achieved, and the transformation it brought to the team. If you have not read it, I would recommend checking it out for the full picture.</p><p>This article is different. This is about the <mark>ugly side</mark> — the real mistakes I made along the way and the lessons I learned from each one. If you are building your own test automation framework, or thinking about it, I hope my experience saves you some pain.</p>',
      sections: [
        {
          heading: "Mistake 1 — Writing Brittle, Unstable Selectors",
          content:
            '<p><strong>What I did wrong:</strong></p><p>When I first started writing tests, I grabbed whatever selector was easiest to find. Class names, auto-generated IDs, deeply nested CSS paths — anything that made the test work right now. My selectors looked something like <mark>div.main-container > div:nth-child(3) > button.btn-primary</mark>. They worked. Until they didn\'t.</p><p><strong>Why it was a problem:</strong></p><p>The moment a developer changed the layout — even something as simple as adding a new div wrapper or reordering elements — my tests broke. Not one or two tests. <mark>Dozens of them.</mark> I remember one morning I came in, ran the test suite, and 15 tests had failed overnight. Nothing in the application was actually broken — a developer had just refactored some CSS classes. Our staging environment was showing all green from a user perspective, but my test dashboard was a sea of red.</p><p>The worst part? These failures looked exactly like real bugs. I wasted hours investigating each one before realizing the app was fine — my selectors were just too fragile.</p><p><strong>How I fixed it:</strong></p><p>I introduced <strong>data-cy attributes</strong> across the application. Instead of relying on CSS classes or DOM structure, every important element got a dedicated test attribute like <mark>data-cy="submit-button"</mark> or <mark>data-cy="email-input"</mark>. These attributes exist purely for testing — developers know not to remove them, and no styling change can break them.</p><p>I also created a shared selectors file so every test referenced selectors from one central place. If something did change, I only had to update it once.</p><p><strong>What I learned:</strong></p><p>Your selectors are the foundation of your entire test suite. <mark>If the foundation is shaky, everything built on top will crumble.</mark> Invest the time upfront to build stable, resilient selectors. It is boring work, but it pays off every single day.</p>',
        },
        {
          heading: "Mistake 2 — Poor Test Structure from the Start",
          content:
            '<p><strong>What I did wrong:</strong></p><p>In the beginning, I just wanted tests to work. I did not think much about organization. All my test files sat in one flat folder. Helper functions were scattered inside individual test files. Some files were 400 lines long with multiple unrelated tests crammed together. There was no naming convention — I had files called <mark>test1.spec.js</mark>, <mark>loginTest.spec.js</mark>, and <mark>campaignFlowNew_v2.spec.js</mark> all living side by side.</p><p><strong>Why it was a problem:</strong></p><p>When the framework grew to 10+ test suites, finding anything became a nightmare. I could not remember which file tested what. When a test failed in CI, I had to open three or four files before finding the right one. New team members who looked at the test folder had no idea where to start. One developer told me, <em>"I wanted to add a test for my feature, but I could not figure out where it should go."</em> That comment hit hard.</p><p><strong>How I fixed it:</strong></p><p>I stopped everything and <strong>restructured the entire framework</strong>. I organized tests by feature — signup tests in one folder, campaign tests in another, billing tests in their own space. I adopted the <mark>Page Object Model</mark> pattern, where each page of the application has its own file describing its elements and actions. Helper functions moved into a shared utilities folder. Test data went into fixtures.</p><p>The folder structure became self-documenting. Anyone could look at it and immediately understand what was tested and where to find it.</p><p><strong>What I learned:</strong></p><p><mark>Structure is not something you add later — it is something you build first.</mark> A well-organized framework with 5 tests is infinitely better than a chaotic one with 50. The five minutes you spend thinking about structure before writing a test will save you five hours of confusion later.</p>',
        },
        {
          heading: "Mistake 3 — Not Handling Waits Properly",
          content:
            '<p><strong>What I did wrong:</strong></p><p>This was the mistake that haunted me the longest. In my early tests, whenever something did not load fast enough, I would add a <mark>cy.wait(3000)</mark> — just wait 3 seconds and hope the page has loaded by then. At one point, I had over 40 hardcoded waits scattered across the framework. Some tests had multiple waits stacked on top of each other.</p><p><strong>Why it was a problem:</strong></p><p>Two things happened. First, the tests became <strong>painfully slow</strong>. All those 2-second and 3-second waits added up. A test suite that should have taken 5 minutes was taking 12 minutes because of all the idle waiting.</p><p>Second — and worse — the tests became <mark>flaky</mark>. On my local machine with a fast connection, 3 seconds was more than enough. But on the CI server? Sometimes it was not. A slow API response or a heavy page load meant 3 seconds was not enough, and the test would fail. Other times the page loaded in 500 milliseconds, and the test would waste 2.5 seconds doing absolutely nothing. I was dealing with random test failures almost daily, and every time I investigated, the root cause was a hardcoded wait that was either too short or too long.</p><p>Our staging environment became the worst offender. It was shared with other teams and sometimes ran slower than usual. On those days, <mark>half my tests would fail</mark> — not because of bugs, but because of timing.</p><p><strong>How I fixed it:</strong></p><p>I went through the entire framework and replaced every single <mark>cy.wait()</mark> with <strong>smart, dynamic waits</strong>. Instead of saying "wait 3 seconds," I taught the tests to say "wait until this element is visible" or "wait until this API call returns a response." Cypress has excellent built-in commands for this — <strong>cy.intercept()</strong> to watch for network requests, and assertion-based waiting that automatically retries until a condition is met.</p><p>For example, instead of:</p><p><mark>cy.wait(3000)</mark><br/><mark>cy.get(\'.dashboard\')</mark></p><p>I wrote:</p><p><mark>cy.intercept(\'GET\', \'/api/dashboard\').as(\'loadDashboard\')</mark><br/><mark>cy.wait(\'@loadDashboard\')</mark><br/><mark>cy.get(\'.dashboard\').should(\'be.visible\')</mark></p><p>The tests now wait for exactly the right moment — no more, no less.</p><p><strong>What I learned:</strong></p><p><mark>Hardcoded waits are the number one enemy of reliable tests.</mark> Every single one is a ticking time bomb. If you catch yourself writing cy.wait(2000), stop and ask: "What am I actually waiting for?" Then wait for that specific thing. Your tests will be faster, more reliable, and much easier to debug.</p>',
        },
        {
          heading: "Mistake 4 — Overcomplicating the Framework",
          content:
            '<p><strong>What I did wrong:</strong></p><p>As I got more comfortable with Cypress, I started getting... ambitious. I built a custom command for everything. I created abstract base classes for page objects. I added a configuration layer on top of Cypress\'s own configuration. I wrote utility functions that wrapped other utility functions. At one point, I had a <mark>helper function that called another helper function that called a Cypress command that did one simple thing</mark> — click a button.</p><p>I thought I was being a "good engineer" by making everything flexible and reusable. I was proud of how "clean" and "abstract" the code looked.</p><p><strong>Why it was a problem:</strong></p><p>When a test failed, I could not figure out what went wrong. The stack trace would point to a utility function, which called another utility, which called a custom command, which eventually did the actual thing. <strong>Debugging became a maze.</strong> I spent more time tracing through my own abstractions than actually fixing tests.</p><p>Even worse, when other team members tried to write tests, they were completely lost. One developer spent an entire afternoon trying to understand my "framework within a framework" before giving up and asking me to just write the test for them. That was a <mark>wake-up call</mark>.</p><p><strong>How I fixed it:</strong></p><p>I stripped it back. Aggressively. I removed every abstraction that did not clearly save time or reduce duplication. If a custom command was only used in two places, I inlined it. If a utility function just wrapped a Cypress command without adding real value, I deleted it. I followed a new rule: <strong>if someone new to the project cannot understand what a test does within 30 seconds of reading it, it is too complicated.</strong></p><p>The framework went from "clever" to <mark>readable</mark>. And readability turned out to be far more valuable than cleverness.</p><p><strong>What I learned:</strong></p><p><mark>Simplicity is not the opposite of good engineering — it is the goal of good engineering.</mark> A test framework should be so straightforward that anyone on the team can read a test, understand it, and write a new one without needing a guided tour. Every layer of abstraction you add is a layer of complexity someone else will have to untangle.</p>',
        },
        {
          heading: "Mistake 5 — Ignoring Reusability Until It Was Too Late",
          content:
            '<p><strong>What I did wrong:</strong></p><p>In my rush to get tests written and coverage numbers up, I kept <strong>copy-pasting code</strong> between tests. Need a login step? Copy it from the other test. Need to create a campaign? Copy that block too. Need to verify a toast notification? Copy, paste, done. I told myself I would clean it up later. <em>"Later"</em> kept getting pushed back.</p><p><strong>Why it was a problem:</strong></p><p>When Beambox updated their login flow — adding a new "remember me" checkbox and changing a field label — I had to update the login steps in <mark>14 different test files</mark>. Fourteen. I spent an entire afternoon making the same change in 14 places, testing each one, and making sure I did not miss any. And I still missed one, which caused a test failure the next morning that took me 20 minutes to track down.</p><p>The copy-paste approach also meant inconsistency. Some tests waited for a success message after login, others did not. Some checked for error states, others skipped it. The "same" login flow was implemented slightly differently in every file, which made the tests behave unpredictably.</p><p><strong>How I fixed it:</strong></p><p>I created <strong>Cypress custom commands</strong> for every repeated action. <mark>cy.login()</mark>, <mark>cy.createCampaign()</mark>, <mark>cy.verifyToast()</mark> — each one defined once and used everywhere. I also built page object files that encapsulated all interactions with a specific page. When the login flow changed again (and it did), I updated it in <strong>one place</strong> and every test that used it was automatically fixed.</p><p>I ended up with 38 reusable components — custom commands, page objects, and utility functions — that turned writing new tests from a 30-minute task into a <mark>5-minute task</mark>.</p><p><strong>What I learned:</strong></p><p>The phrase <em>"I will clean it up later"</em> is the most dangerous sentence in software engineering. <mark>If you are copying code, you are creating a maintenance nightmare.</mark> Build reusable components from day one. Not because it is the "right" thing to do in theory — but because future you will be furious if you do not.</p>',
        },
        {
          heading: "Mistake 6 — Not Thinking About Scalability Early",
          content:
            '<p><strong>What I did wrong:</strong></p><p>When I started, I thought: <em>"We just need to test a few critical flows. How big can this get?"</em> So I made decisions based on a small framework. Test data was hardcoded directly into test files. Configuration was minimal. There was no tagging or grouping — every time the suite ran, <strong>every single test</strong> ran. All of them. Every time.</p><p><strong>Why it was a problem:</strong></p><p>The framework grew faster than I expected. Within four months, I had 21 test suites. Running all of them took <mark>over 40 minutes</mark>. Developers did not want to wait 40 minutes for feedback on a small change to the billing page — they only cared about billing tests, not the entire suite.</p><p>Hardcoded test data became another headache. The staging environment and production environment had different URLs, different test accounts, and different configurations. I was manually changing values every time I switched environments. Once, I accidentally ran production test data against staging and created a bunch of garbage records that the team had to clean up. <mark>That was not a fun conversation.</mark></p><p><strong>How I fixed it:</strong></p><p>I introduced <strong>environment-based configuration</strong> using Cypress environment variables and config files. Each environment — local, staging, production — had its own configuration. Switching environments became a single command flag instead of manual edits.</p><p>I also added <strong>test tagging</strong> so we could run subsets of the suite. Need to test just billing? Run the billing tag. Quick smoke test before a release? Run the smoke tag. Full regression? Run everything. This gave the team <mark>flexibility without sacrificing coverage</mark>.</p><p>For test data, I moved everything into <strong>fixture files</strong> organized by environment. Each fixture contained the right data for its environment, and the framework automatically loaded the correct one based on the config.</p><p><strong>What I learned:</strong></p><p><mark>Always build as if the framework will be 10x bigger than you think.</mark> You do not need to implement everything upfront, but you need to make decisions that do not block you later. Hardcoding values, skipping configuration, and ignoring test organization are shortcuts that become <strong>roadblocks</strong> as the framework grows.</p>',
        },
        {
          heading: "Mistake 7 — Not Investing in Reporting and Visibility",
          content:
            '<p><strong>What I did wrong:</strong></p><p>For the first few weeks, the only way to know if tests passed or failed was to check the <strong>Cypress terminal output</strong>. A wall of green and red text. I understood it because I wrote the tests. Nobody else on the team looked at it. Nobody asked about test results. Nobody cared. The tests existed in a bubble — doing their job silently, but having <mark>zero impact on team behavior</mark>.</p><p><strong>Why it was a problem:</strong></p><p>If nobody sees the results, nobody trusts the system. Developers would merge code without checking test outcomes. Product managers had no idea what was tested and what was not. When I flagged a test failure, people would shrug and say, <em>"Are you sure it is not just a flaky test?"</em> The framework was technically working but <strong>culturally invisible</strong>.</p><p><strong>How I fixed it:</strong></p><p>I set up <strong>Slack notifications</strong> that posted test results directly into the team channel after every CI run. Pass or fail, the team saw it. I added <strong>screenshots and video recordings</strong> of failed tests so developers could see exactly what happened without running anything locally. I integrated test results into <strong>pull request checks</strong> — a failing test blocked the merge, and the failure details were right there in the PR.</p><p>I also built simple dashboards that showed trends over time — <mark>how many tests we had, pass rates, flaky test counts, and coverage by feature</mark>. Suddenly, test health became a team metric, not just a QA metric.</p><p><strong>What I learned:</strong></p><p><mark>A test that nobody sees is a test that nobody trusts.</mark> Visibility is not a nice-to-have — it is what transforms a testing framework from a QA tool into a team culture. The moment test results become visible, shared, and blocking, the entire team starts caring about quality. That is the real win.</p>',
        },
        {
          heading: "Key Takeaways",
          content:
            '<p>If you take nothing else from this article, remember these:</p><ul><li><strong>Stable selectors are non-negotiable.</strong> Use data-cy attributes. Do not rely on CSS classes or DOM structure that can change without warning.</li><li><strong>Structure your framework before you write your second test.</strong> Page Object Model, clear folders, consistent naming. Do it early or pay for it later.</li><li><strong>Never use hardcoded waits.</strong> Every cy.wait(3000) is a future flaky test. Wait for specific conditions, not arbitrary time.</li><li><strong>Keep it simple.</strong> If a new team member cannot understand your test in 30 seconds, it is too complex. Readability beats cleverness every time.</li><li><strong>Build reusable components from day one.</strong> Custom commands, page objects, and shared utilities are not premature optimization — they are survival tools.</li><li><strong>Design for scale.</strong> Environment configs, test tagging, fixture files. Build as if the framework will be 10x bigger than you think.</li><li><strong>Make results visible.</strong> Slack notifications, PR checks, dashboards. If the team cannot see the results, the framework is invisible.</li></ul>',
        },
        {
          heading: "What I Would Do Differently Today",
          content:
            '<p>If I were starting from scratch today, knowing everything I know now, here is exactly what I would do:</p><p><strong>Day one: set up the structure.</strong> Before writing a single test, I would create the folder structure, configure environments, set up the Page Object Model, and establish naming conventions. I would write a short README so anyone joining the project later knows how things work. <mark>The first commit would be the skeleton — not the first test.</mark></p><p><strong>Day two: build the core reusable commands.</strong> Login, logout, navigation, API intercepts, toast verification — all the actions that every test will eventually need. I would invest a full day in these building blocks because they pay dividends on every test written afterward.</p><p><strong>Day three: write the first real test.</strong> One critical flow, end to end, using the structure and commands I just built. This test becomes the <mark>reference implementation</mark> — the example that every future test follows.</p><p><strong>From there: scale with discipline.</strong> Every new test follows the same pattern. Every new page gets a page object. Every repeated action becomes a command. No exceptions, no shortcuts, no "I will clean it up later."</p><p>I would also set up <strong>CI integration and Slack reporting from week one</strong> — not as an afterthought. Visibility from the very beginning changes how the team interacts with the framework. It stops being "the QA person\'s thing" and starts being <mark>"our quality system."</mark></p><p>And honestly? I would be kinder to myself about making mistakes. Because the truth is, <strong>you will make mistakes no matter how much you prepare.</strong> The goal is not to be perfect — it is to learn fast, fix fast, and build something better each time.</p>',
        },
      ],
      conclusion:
        '<p>Building a Cypress framework from scratch was one of the hardest things I have done in my career. But it was also <mark>one of the most valuable</mark>.</p><p>Every mistake I made taught me something I could not have learned from a tutorial. The flaky selectors taught me about resilience. The messy structure taught me about discipline. The hardcoded waits taught me about patience — the right kind of patience. The overcomplicated abstractions taught me that <strong>simplicity is a skill, not a shortcut</strong>.</p><p>Today, I approach test automation very differently than I did when I started. I think about structure before I think about tests. I think about the team before I think about coverage numbers. I think about maintainability before I think about speed.</p><p>If you are early in your automation journey, I want you to know this: <mark>making mistakes does not mean you are doing it wrong. It means you are doing it for real.</mark> The engineers who never make mistakes are the ones who never build anything from scratch. The ones who grow are the ones who build, break, learn, and build again.</p><p>That is the cycle. And honestly? <strong>I would not trade it for anything.</strong></p><p>If any of this resonated with you, or if you are going through similar challenges, I would love to connect. Share your story — I am always learning too.</p>',
    },
  },
  {
    id: "13",
